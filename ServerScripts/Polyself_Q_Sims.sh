...
# 	sleep 10
# fi
slim -d s=$SEL -d h=$DOM -d sfrate=$SELF -d nt=$NTR -d rep=$REP -d isnm=$ISNM -d runtime=$RT -d stype=$STYPE /data/hartfield/polyself/scripts/Polygenic_Selection_With_Selfing.slim
