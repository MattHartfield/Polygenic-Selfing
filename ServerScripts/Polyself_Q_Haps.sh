...
do
	...
	if [ $NL -gt 0 ]
	then
		# (2) Info file with QTLs only, sorted by position
		...
		# (3) Join new VCF, info files; delete old files
		...
		# (4) From new file, calculate total QTL per site per individual
		...
		# (5) Print off (i) summed QTLs (ii) summed QTL effects per individual to file
		for i in $(seq 1 50)
		do
