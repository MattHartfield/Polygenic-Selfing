...
for(a in 0)
{
	...
	for(b in c(1,4))
	{
		...
		for(S in self)
		{
			...
			dat <- read.table(paste0("/scratch/mhartfield/polyself_out/data/polyself_out_s",s,"_h",h,"_self",S,"_nt",N,"_msd",msd,"_isnm",a,"_mscale",b,"_rep",1,".dat"),head=T)
			...
			for(j in 2:reps)
			{
				dat <- read.table(paste0("/scratch/mhartfield/polyself_out/data/polyself_out_s",s,"_h",h,"_self",S,"_nt",N,"_msd",msd,"_isnm",a,"_mscale",b,"_rep",j,".dat"),head=T)
				dat <- dat[ intersect(which(dat$Generation <= (tchange + clup + 1)),which(dat$Generation >= tchange)),]
		...
		for(S in self)
		{
			if(which(self%in%S) == 1){
				...
				legend("bottom",legend=c("S = 0", "S = 0.5", "S = 0.9", "S = 0.999"),col=pcol,lty=1,lwd=1.5,cex=1.15,pt.cex=1)				
			}
			else
			{
